...
<h2 id="{!! md5($endpoint->name()) !!}">{{ $endpoint->name() }}</h2>
...
<span id="execution-results-{{ $endpoint->endpointId() }}" hidden>
    ...
    <pre
        class="json"><code id="execution-response-content-{{ $endpoint->endpointId() }}"
...
<form id="form-{{ $endpoint->endpointId() }}" data-method="{{ $endpoint->httpMethods[0] }}"
    data-path="{{ $endpoint->uri }}" data-authed="{{ $endpoint->isAuthed() ? 1 : 0 }}"
    data-hasfiles="{{ $endpoint->hasFiles() ? 1 : 0 }}" data-isarraybody="{{ $endpoint->isArrayBody() ? 1 : 0 }}"
    autocomplete="off" onsubmit="event.preventDefault(); executeTryOut('{{ $endpoint->endpointId() }}', this);">
    <h3>
...
@if(count($endpoint->responseFields))
    ...
    <x-scribe::nested-fields :fields="$endpoint->nestedResponseFields" :endpointId="$endpoint->endpointId()"
        :isInput="false" />
@endif